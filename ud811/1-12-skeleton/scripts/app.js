(function() {
  ...
  const schemaBuilder = lf.schema.create('city', 1);
  var citiesDB;
  document.addEventListener('DOMContentLoaded', () => {
    ...
    schemaBuilder.connect().then( db => {
      citiesDB = db;
      getAllCities().then(
        res => res.map(data => app.getForecast(data.city, data.label))
      ).catch(
  ...
  function getAllCities() {
    if (citiesDB) {
      var table = citiesDB.getSchema().table('Cities');

      return citiesDB.select().from(table).exec();
    }
  }
  ...
  function saveCity(data) {
    if (citiesDB) {
      ...
      return citiesDB.insertOrReplace().into(table).values([row]).exec();
    }
  }
